...
class Job():
    ...
    def run(self):
        ...
        command = f'ssh {self.username}@{self.host} "cd {self.directory} && chmod +x {self.name}.sh"'  # ug + x was the original one and then there was a ./ in front of the file
        os.system(command)
        time.sleep(1)
        if os_is_windows():

            command = f'ssh {self.username}@{self.host} "cd {self.directory} && nohup ./{self.name}.sh > test-output/{self.name}.log 2> test-output/{self.name}.error"'
            print(command)
    ...
    def get_status(self, refresh=False):
        if refresh:
            ...
            log = readfile(f"{self.output_directory}/{self.name}.log")
    ...
    def get_error(self):
        command = f"scp {self.username}@{self.host}:{self.directory}/{self.name}.error {self.output_directory}/{self.name}.error"
        print(command)
        os.system(command)
    ...
    def get_log(self):
        command = f"scp {self.username}@{self.host}:{self.directory}/{self.name}.log {self.output_directory}/{self.name}.log"
        print(command)
        os.system(command)
        content = readfile(f"{self.output_directory}/{self.name}.log")
        return content

    def sync(self):
        self.mkdir_experimentdir()
        command = f"cd {self.test_directory} && scp ./{self.name}.sh {self.username}@{self.host}:{self.directory}/."
        print(command)
        r = os.system(command)
        return r
